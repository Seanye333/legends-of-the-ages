...
pub fn platform_webview_version() -> Result<String> {
  // Local patch (ThreeKingdomMastersIOS): on iOS/macOS 26 the call
  // `NSBundle::bundleWithIdentifier("com.apple.WebKit")` over-releases inside
  // CoreFoundation (`__CFBundleCopyFrameworkURLForExecutablePath` -> CFRelease),
  // crashing with EXC_BREAKPOINT during startup. This value is informational
  // only, so return a placeholder instead of querying the WebKit bundle.
