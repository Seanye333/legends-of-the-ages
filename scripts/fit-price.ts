...
// 【只拟合最常见的几个 op —— 这不是偷懒,是这份数据能承受的自由度】
// 拿植入了已知真值的假数据排练过(scripts/_synth 那一轮):
//   噪声 ±0.2pp → 38 个参数全部收敛到真值附近,样本外 ρ +0.061(z = 12.5);
//   噪声 ±3pp   → 已经开始拟合噪声,样本外**更差**(z = −2.4);
//   噪声 ±9pp   → 系数飞到 ±30,彻底不可读。
// 而 60 局/张的实测噪声正是 ±9pp。所以问题不在方法,在自由度:
// 稀有 op 只出现在三五张卡上,它那一列的信息量根本撑不起一个参数,
// 岭回归又必须把标准化后的系数除回原尺度,列一稀疏,除数就极小,系数就炸。
//
// 解法是只给**卡数最多**的那几个 op 各留一个参数,其余固定在默认值。
// 被丢掉的 op 的贡献进了残差 —— 那只让噪声略增,不会引入偏倚(它们和保留列
// 基本不相关)。TOPOPS 可调,默认 8。
...
console.log(
  `\n---- 回归拟合(训练 ${trainIdx.length} 张 / 留出 ${holdIdx.length} 张,λ=${lambda})----\n` +
    `只拟合训练集里卡数最多的 ${FITTABLE.length} 个 op,其余固定在默认值:\n  ` +
