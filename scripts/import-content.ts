...
// 【产物入库,重跑必须一致】
//
// 生成的 cards.gen.ts / lore.gen.ts 是**入库的**(构建不依赖姊妹仓库)。
// 这个决定是对的,但它有一个代价:**它会掩盖源头的漂移** ——
// 姊妹仓库改一批数据,已提交的卡池不会变、CI 也不会红,
// 而下一个人一旦重跑这个脚本,卡池就在他手里当场换了一副。
//
// 2026-07 这里曾挂着一段警告:说重跑会把卡池从 2392 张打成 2207 张、
// 白板率 8.9% → 29.5%、立绘 62.5MB → 34.0MB。那次漂移**后来被接受了** ——
// 现在已提交的产物就是当前姊妹仓库的输出,而那段警告一直留在这里吓人。
// 2026-08-03 实测:重跑产物与已提交的**逐字节一致**。
//
// 为了让「警告过期了没人知道」这件事不再发生,加了 `npm run check-generated`:
// 它重跑这个脚本并断言 generated/ 没有任何 diff,已接进 CI。
// 那道闸门红了,才说明源头真的又漂了 —— 那时候再做决定,而不是靠一段注释。
