...
// 【为什么要先折成简体再匹配 —— 以及它实际值多少】
// 2,180 条生平里有 523 条(24%)混着简体字,而且是句内混:
// 曹操传写「漢相曹參之后」、關羽传写「義薄云天」、諸葛亮传写「躬耕于南陽」。
// 两张表全是繁體正则,碰上简体那半个字就整条不命中。
//
// **但事后量过:折叠只多捞到 4 次命中**(事迹 +1、性格 +3)——
// 因为生平里的简体字集中在 后/于/云/内/余/面 这几个,
// 和词表用的字(守鎮騎射斬謀諫…)几乎不重叠。
// 曹操、劉備、樂進、典韋抽不出事迹的真正原因是**词表太小**
//(機警/權數/任俠/桃園結義/先登/萬夫不當 一个都没有),不是字形。
// 折叠留着:它零成本,而且挡住的是「以后往表里加了一个恰好有简体形的词」。
// 别把功劳记在它头上 —— 那一轮的收益全部来自补词。
//
