...
//
// 【逐套量了一遍:零点不挑卡组,**跨度挑**】(BASE=<0..5> CONTROL=1,各 400 局)
//
//   预组        Δ 跨度            中位
//   桃園仁德    −2.3 ~ +6.8      −1.0
//   魏武揮鞭    −6.3 ~ +6.0      +1.3
...
//   大隱於市    −9.0 ~ +1.0      −2.8
//
// 六个中位全部落在 −2.8 ~ +1.8 —— **零点是同一个**,拿 桃園仁德 量出来的那条线
// 用在别的主义上是站得住的(这正是那十一张调平卡需要的前提,它们分布在五套预组里)。
//
// 但**跨度**从 8pp(鷹視狼顧)到 24pp(坐斷東南),差三倍。查下去发现原因不在待测卡:
// **Δ 是个差值,被换掉的那张同样决定结果。** `swapIn` 换的是费用最接近的两张,
// 在 坐斷東南 里 5 费那一档正好是 王世充 和 **陸抗** —— 而 陸抗 单独量是 +21.5。
// 于是「拿一张白板 4/7 换掉陸抗」自然是 −15.8,那量的是**陸抗的价值**,不是白板的。
//
// 两条推论:
//   1. 判一张卡时**看中位那条线,别看跨度** —— 跨度里混着「它换掉了谁」。
//   2. 单卡 Δ 的不确定性**逐套不同**,而校正线用的是全局标准误;
//      在牌组骨架很紧的那几套(坐斷東南)里,它偏乐观。
//
// 【补偿落地之后重量的结果:零点几乎没动】
//   补偿前  −4.8 ~ +6.5,中位 −1.0
//   补偿后  −3.2 ~ +6.7,中位 −1.3
