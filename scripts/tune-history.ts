...
// 一个候选 = (场次, tier, hp)。任务切到局段,理由同 sim-history。
interface Cand {
  battle: number
  tier: number
  hp: number
...
for (const { b, i } of picked) {
  for (const tier of TIERS) {
    for (const d of HP_STEPS) cands.push({ battle: i, tier, hp: b.hp + d })
  }
}
...
cands.forEach((c, ci) => {
  for (let from = 0; from < GAMES; from += CHUNK) {
    jobs.push({ battle: c.battle, from, to: Math.min(from + CHUNK, GAMES), ov: { hp: c.hp, tier: c.tier } })
    jobCand.push(ci)
  }
})

console.log(
  `tune-history: ${picked.length} 场 × ${TIERS.length} 档 tier × ${HP_STEPS.length} 档 hp` +
    ` × ${GAMES} 局 = ${cands.length * GAMES} 局\n`,
)
...
for (const { b, i } of picked) {
  ...
  for (const d of HP_STEPS) {
    const row = TIERS.map((t) => {
      const ci = cands.findIndex((c) => c.battle === i && c.tier === t && c.hp === b.hp + d)
      const v = pct[ci]
      const mark = v > HIGH ? '!' : v < LOW ? '?' : ' '
      return `${v.toFixed(0)}%${mark}`.padStart(7)
    })
    console.log(`hp=${String(b.hp + d).padStart(3)}  ` + row.join(''))
  }
  // 空旋钮检测。hp 对某些场次是**可证明的**零效果 —— 譬如 `objective.kind === 'survive'`:
  ...
  const rowsSame =
    ...
    HP_STEPS.every((d) =>
      TIERS.every((t) => {
        const a = pct[cands.findIndex((c) => c.battle === i && c.tier === t && c.hp === b.hp + d)]
        const z = pct[cands.findIndex((c) => c.battle === i && c.tier === t && c.hp === b.hp + HP_STEPS[0])]
        return a === z
      }),
    )
  if (rowsSame) {
    console.log(
      `  ⓘ hp 这一轴逐格零方差 —— 对这一场它不是弱旋钮,是**空旋钮**,别再在它上面撒网格。` +
  ...
  console.log(
    `  → 建议 hp=${best.c.hp} tier=${best.c.tier.toFixed(2)}(${pct[best.ci].toFixed(0)}%)` +
      (inBand.length ? '' : '  ⚠ 整张网格没有一格落进带里 —— 该动的多半是开局态势,不是这两个旋钮') +
