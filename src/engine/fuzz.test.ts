...
describe('fuzz: random legal games', () => {
  // 【为什么切成四段,而不是一个 it 跑一百局】(2026-08-06)
  // 覆盖面**一模一样**(种子 1..100,一局不多一局不少),切开只为一件事:
  // vitest 的 worker 是单线程的,一个 it 连续占住事件循环 80 多秒时,
  // 它就没法响应主进程的 `onTaskUpdate` RPC,于是整轮 `npm test` 末尾挂一条
  //
  //     Vitest caught 1 unhandled error … Timeout calling "onTaskUpdate"
  //
  // 那不是测试失败(1133 条全绿),是**汇报通道**超时。但它长得就像一条错误,
  // 而这个仓库最不需要的就是一条「每次都在、每次都不用管」的红字 ——
  // 那正是训练所有人忽略错误的东西(同一条道理见 perf-budget 那道闸门的注释)。
  //
  // 每段 25 局约 20 秒,worker 在段与段之间有机会喘一口气,那条 RPC 就不会超时了。
  // 注意**不是**调大某个超时:测试自己的 180s 余量本来就没被顶到,顶到的是别人。
  for (const [from, to] of [
    ...
    it(
      `seeded games ${from}–${to} terminate with all invariants held`,
      () => {
        for (let seed = from; seed <= to; seed++) {
          runFuzzGame(seed)
        }
      },
      // 180s 而不是 60s。2026-08-04 顶穿过一次:全量并行跑 60s 超时,
      ...
      //
      // 所以不去追那个 2.4 倍,而是把余量给够:它的价值在于不变量
      // (百局全跑完、状态始终自洽),不在于跑得快。
      // 一道会随机翻红的闸门等于没有闸门 —— 而且比没有更糟,因为红了没人当真。
      // (切成四段之后每段只跑 25 局,这个余量更是绰绰有余。)
      180_000,
    )
  }
