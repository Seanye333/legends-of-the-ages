...
export const PACK5_CHOOSE_OVERRIDES: Record<string, Partial<CardDef>> = {
  ...
  //
  // 【2026-08-06:补上 keywords: [] —— 这一行漏了三个卡包,它是全卡池最超模的一张】
  // 这一条覆盖是要**替换掉** pack3 那版设计(7 费 6/6,鐵壁 + 衝鋒)的,
  // 上面那句注释也说了「原 7 费 6/6」。但当时只清了 `battlecry`,**没清 `keywords`** ——
  // 而覆盖层是逐字段合并的,于是合出来的是:
  //   6 费(比原设计还便宜 1)· 5/6 · 鐵壁 + 衝鋒 · 外加抉择「守護 / 4 点点杀」
  // 冲锋立刻打 5 点、铁壁扛下反击、再送一个 4 点解场。
  // sim-cards 实测 **Δ +29.2(600 局,z=10.1)—— 全卡池第一**。
  //
  // 还有一个只要多看一眼就该发现的破绽:抉择的第一路是「获得守护」,
  // 而这张牌**已经带着冲锋和铁壁** —— 给一个已经能冲脸的单位加守护,
  // 这个选项在原设计里根本不成立。自相矛盾的卡面就是覆盖没清干净的指纹。
  'jiang-wei': {
    cost: 6,
    attack: 5,
    health: 6,
