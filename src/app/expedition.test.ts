...
describe('远征 run 状态机', () => {
  it('开局 → 打赢 → 亮宝物 → 选一个 → 进下一关', () => {
    ...
    // 选完牌进的是**选路**,不是直接进下一关(路线选择是这一版加的)
    expect(afterCard.stage).toBe(0)
    expect(afterCard.routeOffer).not.toBeNull()
    useExpedition.getState().pickRoute(afterCard.routeOffer![0])
    expect(useExpedition.getState().run!.stage).toBe(1) // 选定路线才进第 2 关
  })
  ...
  it('跳过选牌也能往下走(卡组不变)', () => {
    ...
    useExpedition.getState().pickRoute(after.routeOffer![0])
    expect(useExpedition.getState().run!.stage).toBe(1)
  })
  ...
  it('通关最后一关 → 满进度、无残留 run', () => {
    ...
    useExpedition.setState({
      run: { heroId: 'liu-bei', deck: [], stage: BOSSES.length - 1, relics: [], offered: null, stageMod: null, routeOffer: null, rngState: 1 },
    })
  ...
  // 关间流程:打赢 → 选宝物 → 选牌 → **选路** → 才进下一关。
  // 选路是这一版加的:远征此前是「系统给你一个态势,接受它」,路本身没有分叉。
  it('第 1 关无修饰符;选完牌要先选路,选定才进第 2 关', () => {
    const exp = useExpedition.getState()
    exp.start('liu-bei', Array(30).fill('c'))
    ...
    // 停在选路:给了候选,但还没推进关卡
    const offer = useExpedition.getState().run!.routeOffer!
    expect(offer.length).toBeGreaterThanOrEqual(1)
    expect(useExpedition.getState().run?.stage).toBe(0)

    useExpedition.getState().pickRoute(offer[0])
    expect(useExpedition.getState().run?.stage).toBe(1)
    expect(useExpedition.getState().run?.stageMod).toBe(offer[0])
    expect(useExpedition.getState().run?.routeOffer).toBeNull()
  })

  it('没给的路选不了 —— 不能靠传别的 id 跳过难关', () => {
    const exp = useExpedition.getState()
    exp.start('liu-bei', Array(30).fill('c'))
    exp.settle(true)
    useExpedition.getState().pickRelic(useExpedition.getState().run!.offered![0])
    useExpedition.getState().skipCard()
    useExpedition.getState().pickRoute('mod-nonexistent')
    expect(useExpedition.getState().run?.stage).toBe(0)
    expect(useExpedition.getState().run?.routeOffer).not.toBeNull()
  })
  ...
  it('三选一排除已拥有的宝物', () => {
    ...
    exp.start('liu-bei', Array(30).fill('c'))
    useExpedition.setState({
      run: { heroId: 'liu-bei', deck: [], stage: 0, relics: ['relic-jinpai'], offered: null, stageMod: null, routeOffer: null, rngState: 42 },
    })
    useExpedition.getState().settle(true)
