...
- `npm run fit-price` — 拿实测胜率量定价表准不准(`DELTAS=` 指向 `sim-cards` 的 `DUMP`)。实测 ρ = +0.208(z=10.4)—— 那张表**指对了方向**。40 个参数的回归喂不动(身材系数会解成负的,守卫会拦),但**按效果归组过了留出集**(ρ 0.220 → 0.278,z=2.95)。数值为什么还没落地见 ROADMAP 第 3 节
- `npm run synth-deltas` — 造真值已知的假数据,排练上面那道判决。**排练能证伪一个方法,不能证明一件事不可能**(它的信号强度是设出来的)
- `npm run dead-css` — 死样式清单。CSS Modules 的类名是静态可对号的,所以判得准;但**动态取用的模块一律跳过 —— 那是盲区不是「干净」**
- `npm run pool-snapshot` — 重生成卡池快照(`src/content/pool-snapshot.json`)。闸门是 `poolSnapshot.test.ts`,守 `check-generated` 覆盖不到的覆盖层;故意改了卡池就跑它,**把 diff 一起提交 —— 那份 diff 就是改动清单**
...
11. **闸门自己也要能被验证。**(2026-08-04 加,起因是一天之内发现两道闸门在说谎)
    ...
    判定逻辑一律从「跑模拟的脚本」里抽成**纯函数**单放一个模块,再配一份喂合成数据的自检 —— 两个方向都验:**该红时红、不该红时不红**。样板见 `scripts/campaignGate.ts` + `campaignGate.test.ts`,几毫秒跑完,不必等十分钟的模拟。目前八份:`campaignGate` / `simSeating` / `firstPlayerGate` / `balanceGate` / `baseline` / `perfBudgetGate` / `contentRules` / `cssOrder`。同一套做法也用在不卡门的清单上,因为**一把会误报的尺子比没有尺子更危险**(`pricing` / `correlation` / `fitWeights` / `deadCss` / `cssDupes` 各带自检 —— `deadCss` 的第一版就把全站共用的 `uiKit.module.css` 报成了「整个文件都是死的」)。
    ...
    - **先问分辨力,再问结论。**(2026-08-05 加)一道判定可以完全诚实、完全正确,而**分辨力是零** —— 那时候「没问题」和「测不出来」在输出里长得一模一样。量分辨力的办法是**造一份真值已知的假数据**跑一遍(`scripts/synth-deltas.ts`):植入的信号捞不回来,说明这道判定此刻读出的任何东西都不能信。它当场判了「40 个参数的岭回归拟合定价表」死刑 —— 而真实数据上那次回归果然把身材系数解成了负的。**但排练本身也会骗人:** 假数据的信号强度是我随手设的,据此算出的「天花板」比真实数据低一半,差点让我写下「这件事在这个预算下根本做不了」——实际上换成按效果归组就做成了(留出集 z = 2.95)。**排练能证伪一个方法,不能证明一件事不可能。**
