...
| `npm test` | Vitest(引擎/内容/AI/收集/任务/存档/卡组码/成就/竞技场/冒险 152 测试) |
| `npm run lint` | ESLint(引擎纯度围栏 + 各运行时 globals) |
| `npx playwright test` | 浏览器端到端 19 例(对局/图鉴/构筑/开包/教程/军令/回放/主公技/卡组码/设置/竞技场/成就/冒险/墓地) |
...
| `npm run sim-campaign` | 关底战难度曲线 + 三道闸门 |
| `npm run tune-campaign` | 二分搜索关底战难度参数(只打印建议值) |
...
| | 用什么卡组 | 计分 | 一句话 |
|---|---|---|---|
| 天梯 / 随便打 | 自己的收藏 | 天梯计 ELO | 构筑的主场 |
| **竞技场「校场点将」** | **现抽 30 张,不看收藏** | 不计 | 新号老号同一条起跑线 |
| **冒险「群雄逐鹿」** | 自己的收藏 | 首通发奖 | 八场关底战,构筑的试金石 |
...
- **设置页** — 战绩总览、音效/音乐两套音量、减少动效、语言、敌手强度、立即同步、设备 ID、清空进度。
- **功名簿** — 25 条永久成就(军令每天清零,这个不会)。奖励以功勋为主,只有两条最难的给卡包。
- **背景音乐** — 五声音阶实时合成,零音频文件,**不循环固定旋律**(每小节现掷骰子选音)。
- **墓地查看** — 双方已打出/阵亡的牌,同名合并计数。中后期「对手的火計用掉没有」是关键信息。
...
- **观战** — 凭房间码以观战席接入,看得到双方场面与手牌数,**看不到任何一方的手牌牌面**
- **再战** — 终局后三分钟窗口,双方都点才重开;再战局不计天梯分(防刷)
- **赛季** — 天梯按 UTC 自然月分季,换季软重置(`1200 + (旧分-1200)×0.5`),惰性折算不遍历全表
